# -*- coding: utf-8 -*-
"""
Created on Wed Nov  5 19:27:25 2025
...
N_BINS = 50
BOOTSTRAP_SAMPLES = 800  # Increased from 500
PDF_BOOTSTRAP = 80       # Increased from 50
FOURIER_TERMS = 600      # Increased from 500

# Enhanced walker counts
N_WALKERS_FJC = 150000   # Increased from 100000
N_WALKERS_SAW_BASE = 3000 # Base for adaptive sizing (increased from 2000)
N_WALKERS_WLC = 50000    # Increased from 30000

# Enhanced SAW parameters
SIMPLE_MC_TRIALS = 30    # Increased from 20
PERM_K_TRIALS = 25       # Increased from 20
PERM_C_MIN = 0.25        # More conservative pruning (was 0.2)
PERM_POPULATION_FACTOR = 5 # Increased from 4
PIVOT_EQUILIBRATION = 3000 # Increased from 2500
PIVOT_ATTEMPTS_FACTOR = 10 # Increased from 8

# Enhanced WLC parameters
WLC_MAX_ATTEMPTS_FACTOR = 200 # Increased from 150
...
def bootstrap_pdf_errors(data, x_grid, weights=None, n_bootstrap=PDF_BOOTSTRAP, alpha=0.05):
    """
    ...
    pdf_lower = np.percentile(bootstrap_pdfs, 100*alpha/2, axis=0)
...
# ===========================================
# ENHANCED ADAPTIVE SAW SAMPLING FUNCTIONS
# ===========================================
def improved_adaptive_saw_monte_carlo_Px(x0, a, N, L, bead_radius=0.03, n_walkers=N_WALKERS_SAW_BASE,
    ...
    """
    Enhanced Adaptive SAW Sampler with improved sampling strategies for different chain length regimes
    ...
    if N >= 50:
        effective_walkers = 6000  # Increased from 3500
        print(f"Using ENHANCED sampling: N={N} -> {effective_walkers} walkers")
    elif N >= 40:
        effective_walkers = 5000  # Increased from 3000
        print(f"Using ENHANCED sampling: N={N} -> {effective_walkers} walkers")
    elif N >= 30:
        effective_walkers = 4000  # Increased from 2500
        print(f"Using ENHANCED sampling: N={N} -> {effective_walkers} walkers")
    else:
        effective_walkers = n_walkers  # Keep enhanced base for short chains
        print(f"Using ENHANCED sampling: N={N} -> {effective_walkers} walkers")

    # ENHANCED HYBRID STRATEGY WITH OPTIMIZED PARAMETERS
    if N >= pivot_threshold:
        print(f"Using ENHANCED PIVOT algorithm for SAW N={N}")
        return improved_pivot_saw(x0, a, N, L, bead_radius, n_samples=effective_walkers)
    elif N >= perm_threshold:
        print(f"Using ENHANCED PERM algorithm for SAW N={N}")
        return improved_perm_saw(x0, a, N, L, bead_radius, n_walkers=effective_walkers)
    else:
        print(f"Using ROBUST Simple MC for SAW N={N}")
...
def robust_simple_saw(x0, a, N, L, bead_radius=0.03, n_walkers=N_WALKERS_SAW_BASE, max_attempts_factor=1000):
    """
    ROBUST Simple SAW Monte Carlo - Enhanced version without clearance optimization bias
    Uses the original reliable approach with improved diagnostics and parameters
    """
    @njit
    def generate_robust_saw_chain(N, a, x0, L, bead_radius, max_trials=SIMPLE_MC_TRIALS):  # Enhanced trials
        coords = np.zeros((N+1, 3))
        coords[0, 0] = x0

        for i in range(1, N+1):
            found_valid = False
            # Try multiple random directions without bias
            for trial in range(max_trials):
                step = np.random.normal(0.0, 1.0, 3)
                step_norm = np.sqrt(np.sum(step**2))
                if step_norm > 1e-8:
                ...
                # Check self-avoidance - use original reliable approach
                collision = False
                for j in range(i):
                    if np.linalg.norm(new_pos - coords[j]) < 2 * bead_radius:
                        collision = True
    ...
    while len(ends) < n_walkers and attempts < max_attempts:
        ...
        if attempts % progress_interval == 0 and len(ends) > 0:
            ...
            print(f"Enhanced Simple SAW N={N}: Progress {len(ends)}/{n_walkers}, "
    ...
    if len(ends) < n_walkers:
        print(f"⚠️ Enhanced Simple SAW at N={N}: {len(ends)} of {n_walkers} accepted, "
              f"acceptance rate: {acceptance_rate:.6f}, ESS: {ess}")
    else:
        print(f"✅ Enhanced Simple SAW at N={N}: All {n_walkers} samples accepted, "
...
def improved_perm_saw(x0, a, N, L, bead_radius=0.03, n_walkers=N_WALKERS_SAW_BASE,
    ...
    """
    Enhanced PERM algorithm with improved weight management and population control
    """

    # Adaptive parameters based on chain length
    if N >= 35:
        k_trials = 30  # More trials for longer chains in PERM regime (increased from 25)
        c_min = 0.20   # More lenient pruning for very long chains
        print(f"Enhanced PERM SAW N={N}: Using extended-chain parameters")
    ...
    print(f"Enhanced PERM SAW N={N}: Starting with {len(population)} walkers, "
    ...
    while current_step <= N and len(population) > 0:
        ...
        for chain, weight, walker_id in population:
            ...
            if len(valid_steps) > 1:
                ...
                for step in valid_steps:
                    min_dist = 1e10  # Use large number instead of float('inf')
        ...
        if len(new_population) == 0:
            print(f"⚠️ Enhanced PERM SAW N={N}: All chains trapped at step {current_step}")
            break
        ...
        if current_step % 5 == 0:  # More frequent reporting for PERM
            ...
            print(f"Enhanced PERM SAW N={N}: Step {current_step}, population: {len(population)}, "
    ...
    print(f"Enhanced PERM SAW N={N}: {complete_chains} complete chains, "
...
def improved_pivot_saw(x0, a, N, L, bead_radius=0.03, n_samples=N_WALKERS_SAW_BASE,
    ...
    """
    Enhanced pivot algorithm with better equilibration and sampling
    """
    # Generate initial chain with more attempts for reliability
    print(f"Enhanced Pivot SAW N={N}: Generating initial chain...")
    initial_chain = generate_initial_saw_chain(x0, a, N, L, bead_radius, max_attempts=20000)

    if initial_chain is None:
        print(f"❌ Enhanced Pivot SAW N={N}: Failed to generate initial chain")
    ...
    print(f"Enhanced Pivot SAW N={N}: Starting pivot moves (equilibration: {n_equilibration})")
    ...
    for step in range(n_equilibration):
        ...
        if step % 200 == 0 and len(recent_acceptances) == window_size:
            ...
            if recent_rate < 0.1:
                ...
                print(f"Enhanced Pivot SAW N={N}: Reduced max_angle to {max_angle:.3f}")
            elif recent_rate > 0.3:
                max_angle = min(max_angle * 1.2, np.pi)  # Increase angle for larger moves
                print(f"Enhanced Pivot SAW N={N}: Increased max_angle to {max_angle:.3f}")

        if (step + 1) % 1000 == 0:
            current_acceptance = accepted / attempts if attempts > 0 else 0
            print(f"Enhanced Pivot SAW N={N}: Equilibration {step+1}/{n_equilibration}, "
                  f"acceptance: {current_acceptance:.4f}")

    equilibration_acceptance = accepted / attempts if attempts > 0 else 0
    print(f"Enhanced Pivot SAW N={N}: Equilibration complete, acceptance: {equilibration_acceptance:.4f}")
    ...
    print(f"Enhanced Pivot SAW N={N}: Generated {len(final_positions)} samples, "
...
def enhanced_wlc_monte_carlo_Px(x0, a, N, L, lp=0.2, temperature_kT=1.0,
    ...
    """
    # Enhanced persistence length adaptation based on chain length
    if N <= 15:
        enhanced_lp = lp * 1.25  # More stiff for short chains (increased from 1.2)
    elif N >= 40:
        enhanced_lp = lp * 0.85  # More flexible for long chains (decreased from 0.9)
    else:
        enhanced_lp = lp
...
def enhanced_analyze_combined_models(N_values, a=0.1, lp=0.2, x0=0.75, L=2.0,
                                    n_walkers_fjc=N_WALKERS_FJC, n_walkers_saw=N_WALKERS_SAW_BASE, n_walkers_wlc=N_WALKERS_WLC):
    """
    ...
    print("="*80)
    print("N < 30: Robust Simple MC (3000 walkers, 30 trials)")
    print("30 ≤ N < 40: Enhanced PERM (4000 walkers, k_trials=25)")
    print("N ≥ 40: Enhanced Pivot Algorithm (5000-6000 walkers)")
    print("FJC: 150,000 walkers | WLC: 50,000 walkers")
    print("Enhanced persistence length adaptation and parameter tuning")
    print("BOUNDARY ENFORCEMENT: P(x) = 0 at x=0 and x=L")
    print("Improved statistics: 800 bootstrap samples, 600 Fourier terms")
    print("="*80)
    ...
    for idx, N in enumerate(N_values):
        ...
        # Enhanced analytical solution
        fourier_pdf = analytical_Px_fourier(x_grid, x0, a, N, L, n_terms=FOURIER_TERMS)
        ...
        saw_pdf_mean, saw_pdf_err = bootstrap_pdf_errors(saw_data, x_grid, saw_weights, n_bootstrap=PDF_BOOTSTRAP)

        # IMPROVED VISUALIZATION WITH ENHANCED SAW ERROR BARS
        # Plot FJC histogram with reduced alpha for better visibility
        ax.hist(fjc_data, bins=60, range=(0,L), density=True, alpha=0.3,
...
if __name__ == "__main__":
    ...
    print("OPTIMIZED SAMPLING STRATEGY:")
    print("  FJC: 150,000 walkers (50% increase)")
    print("  SAW: Adaptive sampling with enhanced walker counts:")
    print("    N < 30: 3000 walkers (50% increase)")
    print("    30 ≤ N < 40: 4000 walkers (60% increase)")
    print("    40 ≤ N < 50: 5000 walkers (67% increase)")
    print("    N ≥ 50: 6000 walkers (71% increase)")
    print("  WLC: 50,000 walkers (67% increase)")
    print("ENHANCED PARAMETERS:")
    print("  Simple MC: 30 trials (50% increase)")
    print("  PERM: k_trials=25, c_min=0.25, population_factor=5")
    print("  Pivot: 3000 equilibration, 10 attempts per step")
    print("  WLC: Enhanced persistence adaptation and 200 max attempts factor")
    print("STATISTICAL IMPROVEMENTS:")
    print("  800 bootstrap samples (60% increase)")
    print("  80 PDF bootstrap samples (60% increase)")
    print("  600 Fourier terms (20% increase)")
    print("BOUNDARY ENFORCEMENT: P(x) = 0 at x=0 and x=L")
    print("MAINTAINED: All robust sampling features and output structure")
    print("=" * 80)
    ...
    print("✅ FJC: Increased to 150,000 walkers (50% improvement)")
    print("✅ SAW: Significantly increased walker counts across all regimes")
    print("✅ WLC: Increased to 50,000 walkers with enhanced parameters")
    print("✅ Enhanced algorithm parameters: More trials, better equilibration")
    print("✅ Improved statistics: More bootstrap samples and Fourier terms")
    print("✅ Maintained robust sampling: All previous fixes preserved")
    print("✅ Boundary enforcement: P(x) = 0 at x=0 and x=L")
    ...
    print("✅ Enhanced persistence length adaptation for WLC")